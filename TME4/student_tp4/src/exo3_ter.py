...
for epoch in tqdm(range(nb_epochs)):
    for X, _ in data_train:
        ...
        h = torch.zeros((pas_temps,X.size(1),X.size(2),latent_dim)).to(device)
        loss=0
        for i in range(X.shape(0)-pas_temps-1):
            print(X.shape)
            print(X[i+1:i+pas_temps+1].shape)
            print(h.shape)
            h = state.model.one_step(X[i], h).to(device)
            ypred = state.model.decode(h).to(device)
            ytrue = X[i+1:i+pas_temps+1].to(device)
            loss += f_cout(ypred, ytrue)
...
#TEST
    with torch.no_grad():
        ...
        h = torch.zeros((pas_temps,X.size(1),X.size(2),latent_dim)).to(device)
        loss =0
        for i in range(X.size(0)-pas_temps-1):
            h = state.model.one_step(X[i], h).to(device)
            ypred = state.model.decode(h).to(device)
            ytrue = X[i+1:i+pas_temps+1]
            loss += f_cout(ypred, ytrue)
