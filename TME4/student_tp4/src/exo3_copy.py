    ...
    for X, y in data_train:
        ...
        for c in range(CLASSES):
            ...
            for i in range(X.size(0)-pas_temps):
                for j in range(pas_temps):
                    h = state.model.one_step(X[i+j,:,c,:], h).to(device)
                yt = state.model.decode(h).to(device)
                loss += f_cout(yt, y[i+pas_temps,:,c,:])
        liste_loss_train.append(loss)
        ...
        for X, y in data_test:
            ...
            for c in range(CLASSES):
                ...
                for i in range(X.size(0)-pas_temps):
                    ...
                    yt = state.model.decode(h).to(device)
                    loss += f_cout(yt, y[i+pas_temps,:,c,:])
            liste_loss_test.append(loss)
            writer.add_scalar("Loss/test", loss, epoch)

plt.plot(liste_loss_train, label="train")
plt.plot(liste_loss_test, label="test")
plt.legend()
plt.show()
